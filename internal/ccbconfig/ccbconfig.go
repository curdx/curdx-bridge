// Package ccbconfig provides CCB configuration for Windows/WSL backend environment.
// Source: claude_code_bridge/lib/ccb_config.py
package ccbconfig
...
// GetBackendEnv returns the BackendEnv from env var or .ccb-config.json.
// Returns "wsl", "windows", or "".
func GetBackendEnv() string {
	v := strings.TrimSpace(strings.ToLower(os.Getenv("CCB_BACKEND_ENV")))
	if v == "wsl" || v == "windows" {
		return v
	}
	...
	path := filepath.Join(cwd, ".ccb-config.json")
