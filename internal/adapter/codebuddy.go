...
// Source: claude_code_bridge/lib/askd/adapters/codebuddy.py
package adapter
...
// CodebuddyAdapter implements BaseProviderAdapter for the CodeBuddy provider.
type CodebuddyAdapter struct{}

func (a *CodebuddyAdapter) Key() string                        { return "codebuddy" }
func (a *CodebuddyAdapter) Spec() providers.ProviderDaemonSpec { return providers.BaskdSpec }
func (a *CodebuddyAdapter) SessionFilename() string            { return ".codebuddy-session" }
func (a *CodebuddyAdapter) OnStart()                           {}
func (a *CodebuddyAdapter) OnStop()                            {}

func (a *CodebuddyAdapter) LoadSession(workDir string, instance string) (any, error) {
	s := session.LoadCodebuddySession(workDir, instance)
	if s == nil {
		return nil, nil
	}
	return s, nil
...
func (a *CodebuddyAdapter) ComputeSessionKey(sess any, instance string) string {
	s, ok := sess.(*session.CodebuddyProjectSession)
	if !ok || s == nil {
		return "codebuddy:unknown"
	}
	return session.ComputeCodebuddySessionKey(s, instance)
}

func (a *CodebuddyAdapter) HandleException(err error, task *QueuedTask) *ProviderResult {
	return DefaultHandleException("codebuddy", err, task)
}

func codebuddyWriteLog(line string) {
	runtime.WriteLog(runtime.LogPath(providers.BaskdSpec.LogFileName), line)
}

// resolveCodebuddyPaneLogPath resolves the pane log path for a CodeBuddy session.
func resolveCodebuddyPaneLogPath(sess *session.CodebuddyProjectSession) string {
...
func (a *CodebuddyAdapter) HandleTask(task *QueuedTask) *ProviderResult {
	startedMs := nowMs()
	req := task.Request
	codebuddyWriteLog(fmt.Sprintf("[INFO] start provider=codebuddy req_id=%s work_dir=%s", task.ReqID, req.WorkDir))

	instance := req.Instance
	sess := session.LoadCodebuddySession(req.WorkDir, instance)
	var sessionKey string
	if sess != nil {
		sessionKey = session.ComputeCodebuddySessionKey(sess, instance)
	} else {
		sessionKey = "codebuddy:unknown"
	}
	...
	paneLogPath := resolveCodebuddyPaneLogPath(sess)

	logReader := comm.NewCodebuddyLogReader(sess.WorkDir(), paneLogPath)
	state := logReader.CaptureState()

	prompt := provprotocol.WrapCodeBuddyPrompt(req.Message, task.ReqID)
	if err := backend.SendText(paneID, prompt); err != nil {
		codebuddyWriteLog(fmt.Sprintf("[ERROR] SendText failed req_id=%s: %v", task.ReqID, err))
		return &ProviderResult{
			ExitCode:   1,
	...
	rebounded := false
	tailBytes := int64(envIntDefault("CCB_BASKD_REBIND_TAIL_BYTES", 2*1024*1024))
	paneCheckInterval := envFloatDefault("CCB_BASKD_PANE_CHECK_INTERVAL", 2.0)
	...
	for {
		...
		if task.CancelEvent != nil {
			...
			case <-task.CancelEvent:
				codebuddyWriteLog(fmt.Sprintf("[INFO] Task cancelled during wait loop: req_id=%s", task.ReqID))
				goto loopEnd
			default:
			}
		}
		...
		if time.Since(lastPaneCheck).Seconds() >= paneCheckInterval {
			...
			if !alive {
				codebuddyWriteLog(fmt.Sprintf("[ERROR] Pane %s died during request req_id=%s", paneID, task.ReqID))
				return &ProviderResult{
					ExitCode:     1,
...
loopEnd:
	...
	codebuddyWriteLog(fmt.Sprintf("[INFO] done provider=codebuddy req_id=%s exit=%d", task.ReqID, result.ExitCode))
	return result
}
