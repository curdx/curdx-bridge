...
- `/ask claude What is 12+12?` (send via heredoc)
- `CURDX_CALLER=codex ask claude <<'EOF'`
  `What is 12+12?`
  `EOF`

## Notes

- If it fails, check backend health with the corresponding ping command (`curdx-ping <provider>` (e.g., `curdx-ping claude`)).
