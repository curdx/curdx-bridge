<#
.SYNOPSIS
  CURDX Installer for Windows — downloads pre-built binaries from GitHub Release.
...
if (-not $InstallDir) {
  $InstallDir = Join-Path $env:LOCALAPPDATA "curdx\bin"
}
$ShareDir = Join-Path $env:LOCALAPPDATA "curdx\share"
...
function Install-Skills {
  ...
  foreach ($obs in @("cask","gask","oask","lask","cpend","gpend","opend","lpend","cping","gping","oping","lping","ping","auto","tp","tr","all-plan","ask","pend","autonew","file-op","mounted","continue","review")) {
...
function Install-ClaudeMd {
  ...
  $template = Join-Path $ShareDir "config\claude-md-curdx.md"
  $startMarker = "<!-- CURDX_CONFIG_START -->"
  $endMarker = "<!-- CURDX_CONFIG_END -->"
  ...
  if (Test-Path $claudeMd) {
    ...
    if ($content -match [regex]::Escape($startMarker)) {
      ...
      Write-Ok "Updated CURDX config in CLAUDE.md"
    } else {
      Add-Content $claudeMd -Value ("`n" + $templateContent)
      Write-Ok "Added CURDX config to CLAUDE.md"
    }
  } else {
    Set-Content $claudeMd -Value $templateContent
    Write-Ok "Created CLAUDE.md with CURDX config"
  }
}

function Install-CURDX {
  Write-Host ""
  Write-Host "  CURDX Installer for Windows" -ForegroundColor White
  Write-Host ""
  ...
  $archive = "curdx-windows-${arch}.zip"
  $url = "https://github.com/$Repo/releases/download/$ver/$archive"

  $tmpDir = Join-Path $env:TEMP "curdx-install-$(Get-Random)"
  New-Item -ItemType Directory -Path $tmpDir -Force | Out-Null

  try {
    Write-Info "Downloading CURDX $ver for windows/$arch..."
    ...
    $srcDir = Join-Path $tmpDir "curdx-windows-${arch}"
    ...
    Write-Host "    curdx --help          # Show help"
    Write-Host "    curdx codex claude    # Start with Codex + Claude"
    Write-Host ""
...
function Uninstall-CURDX {
  Write-Host ""
  Write-Host "  CURDX Uninstaller" -ForegroundColor White
  Write-Host ""

  if (Test-Path $InstallDir) {
    $exes = Get-ChildItem $InstallDir -Filter "*.exe" | Where-Object { $_.Name -match "^(curdx|cxb-|ask|cask|gask|oask|lask|cpend|gpend|lpend|cping|gping|lping|askd|laskd|autoloop|autonew|ctx-transfer|curdx-|pend)\b" }
    foreach ($exe in $exes) {
...
switch ($Command) {
  "install"   { Install-CURDX }
  "uninstall" { Uninstall-CURDX }
}
