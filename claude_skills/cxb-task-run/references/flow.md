...
**Auto-loop daemon**: started by `/cxb-task-plan` (`bash ~/.claude/skills/cxb-task-run/scripts/autoloop.sh start`). `/cxb-task-run` should assume it is running and only ensure the finalize request doesn't stop it.
...
Call `/cxb-file-op` with `FileOpsREQ`:
- Template: `../templates/preflight.json`

Interpret `FileOpsRES`:
- If no plan → show `No plan. Use /cxb-task-plan first.` → Stop
...
Implementation detail: Claude must not read repo files directly; request reads via `/cxb-file-op` (`read_file`) and parse JSON locally.
...
#### 2.2 Codex Independent Design (via `/cxb-ask codex`)

```
/cxb-ask codex "Independent step design:
Step: [title]
...
If valid, apply split via `/cxb-file-op` (use `data.state.current.stepIndex` from Preflight):
...
Send the constructed FileOpsREQ via `/cxb-file-op`:

```
/cxb-file-op <the FileOpsREQ JSON>
```

(`/cxb-file-op` handles routing to Codex via `CURDX_CALLER=claude cxb-ask codex`)
...
- If `executor == "opencode"`:
  - Codex uses the internal `cxb-opencode-ask` skill to call OpenCode.
...
```
Bash(CURDX_CALLER=claude cxb-ask codex "Run tests for this change:
...
- Output: `Split applied. Next: first substep. Use /cxb-task-run (autoloop will trigger if running).`
...
Send `FileOpsREQ` with `purpose: "finalize_step"` via `/cxb-file-op`. Codex returns `FileOpsRES` JSON only.
...
- If there are remaining steps, it must trigger the next `/cxb-task-run` automatically.
...
Output result:
- If more steps: `Step N complete. Next: [title]. Use /cxb-task-run`
...
Invoke `/cxb-review` skill with task mode:
...
- **Medium issues** (need 1-2 extra steps) → Append steps to current task:
  ...
  2. Call `/cxb-file-op` to append steps to `.curdx/state.json`:
     - Template: `../templates/append-steps.json`
  3. Continue executing `/cxb-task-run` to complete appended steps
...
- **Large issues** (need >2 steps) → Create follow-up task:
  ...
  2. Prompt user: "Found N larger issues, suggest creating new task: /cxb-task-plan [follow-up description]"
...
- `documenter = "gemini"`: Claude calls `/cxb-ask gemini` to generate `reportContent` (markdown), then has Codex write the file
