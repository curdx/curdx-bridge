...
Send to `inspiration` (via `/cxb-ask`):
...
WHILE result == FAIL AND iteration <= 3:
  ...
  5. Re-submit to `reviewer` via /cxb-ask (same template)
  6. iteration += 1
...
> Generated by cxb-coplan (`designer` + `inspiration` + `reviewer`)
...
- Resolve `inspiration` and `reviewer` to providers via CLAUDE.md Role Assignment, then use `/cxb-ask <provider>`
