...
- `mode=step`: Review single step execution (used by /cxb-task-run Step 7)
- `mode=task`: Review entire task completion (used by /cxb-task-run Step 9.1)
...
Implementation detail: Claude must not read repo files directly; request file reads via `/cxb-file-op` (`read_file`) and parse the JSON response.
...
If provider is:
- `codex` → use `/cxb-ask codex`
- `gemini` → use `/cxb-ask gemini`

```
/cxb-ask <provider> "Cross-review:
