...
func usage() {
	fmt.Fprintln(os.Stderr, "Usage: cxb-askd [OPTIONS]")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Unified cxb-ask daemon (all providers)")
	fmt.Fprintln(os.Stderr, "")
...
func main() {
	...
	if shutdown {
		sf := stateFile
		if sf == "" {
			sf = runtime.StateFilePath("cxb-askd.json")
		}
		ok := rpc.ShutdownDaemon("cxb-askd", 5.0, sf)
		if ok {
			os.Exit(0)
		}
		os.Exit(1)
