...
func debugEnabled() bool {
	return envutil.EnvBool("CCB_COMPLETION_HOOK_DEBUG", true)
}

func debugLogPath() string {
	raw := strings.TrimSpace(os.Getenv("CCB_COMPLETION_HOOK_DEBUG_LOG"))
	if raw != "" {
	...
	return filepath.Join(os.TempDir(), "ccb-completion-hook.debug.log")
}
...
func findWeztermCLI() string {
	...
	wezBin := strings.TrimSpace(os.Getenv("CCB_WEZTERM_BIN"))
	if wezBin != "" {
...
func sendViaTmux(paneID, message string) bool {
	sendEnter := func() bool {
		...
		retryDelay := envFloat("CCB_TMUX_ENTER_RETRY_DELAY", 0.08)
	...
	bufferName := fmt.Sprintf("ccb-hook-%d-%d", os.Getpid(), time.Now().UnixMilli())
...
func renderTerminalMessage(providerDisplay, reqID, replyContent string, outputFile string, status string) string {
	...
	if outputFile != "" {
		return fmt.Sprintf("CCB_REQ_ID: %s\n\n%s\nProvider: %s\nStatus: %s\nOutput file: %s\n\nResult: %s\n",
...
func run(argv []string) int {
	...
	// CCB_CALLER env takes priority over --caller arg
	caller := strings.ToLower(strings.TrimSpace(os.Getenv("CCB_CALLER")))
	if caller == "" {
	...
	doneSeen := envutil.EnvBool("CCB_DONE_SEEN", true)
	status := completionhook.NormalizeCompletionStatus(os.Getenv("CCB_COMPLETION_STATUS"), doneSeen)
	...
	// Direct pane routing
	directPaneID := strings.TrimSpace(os.Getenv("CCB_CALLER_PANE_ID"))
	directTerminal := strings.TrimSpace(os.Getenv("CCB_CALLER_TERMINAL"))
	...
	if directPaneID != "" {
		...
		workDir := os.Getenv("CCB_WORK_DIR")
		...
		home, _ := os.UserHomeDir()
		if home != "" {
			for _, dirname := range []string{".ccb", ".ccb_config"} {
	...
	if paneID == "" {
		...
		timeout := envFloat("CCB_COMPLETION_HOOK_TIMEOUT", 10.0)
