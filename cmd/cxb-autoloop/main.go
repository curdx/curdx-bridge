// cxb-autoloop - AutoFlow cxb-autoloop daemon: trigger /cxb-task-run when state advances.
//
// Monitors state.json for cursor changes and triggers /cxb-task-run via `cxb-ask claude`.
// Optionally clears context first if usage exceeds threshold.
//
// Source: claude_skills/cxb-task-run/scripts/autoloop.sh
package main
...
func lask(repo, text string) error {
	askPath, err := exec.LookPath("cxb-ask")
	if err != nil {
		home, _ := os.UserHomeDir()
		askPath = filepath.Join(home, ".local", "bin", "cxb-ask")
	}
	env := os.Environ()
	env = append(env, "CURDX_CALLER=cxb-autoloop")
...
func daemon(
	...
	lock, err := acquireLock(lockPath)
	if err != nil {
		fmt.Fprintln(os.Stderr, "cxb-autoloop already running")
		return 0
	}
	defer lock.release()
	...
	for {
		...
		if lastMtime == nil {
			...
			if initial != nil {
				...
				// Auto-trigger /cxb-task-run on first state.json detection if work remains
...
func run() int {
	...
	flag.Usage = func() {
		fmt.Fprintln(os.Stderr, "Usage: cxb-autoloop [options]")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "AutoFlow cxb-autoloop daemon: trigger /cxb-task-run when state advances")
		fmt.Fprintln(os.Stderr, "")
	...
	stateFile := filepath.Join(repo, ".curdx", "cxb-autoloop_state.json")
	lockPath := filepath.Join(repo, ".curdx", "cxb-autoloop.lock")
