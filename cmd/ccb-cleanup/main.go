...
// Usage:
//
//	ccb-cleanup [--list] [--clean] [--kill-zombies]
//
// Source: claude_code_bridge/bin/ccb-cleanup
package main
...
func cleanupStaleStateFiles() []string {
	...
	cacheDir := filepath.Join(home, ".cache", "ccb", "projects")
...
func cleanupStaleLocks() []string {
	home, _ := os.UserHomeDir()
	runDir := filepath.Join(home, ".ccb", "run")
