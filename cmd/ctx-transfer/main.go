...
Options:
  ...
  --from, --source P   Source provider: auto|claude|codex|gemini|opencode|droid (default: auto)
  -t, --to P           Target provider: codex|gemini|opencode|droid (default: codex)
...
var validSourceProviders = map[string]bool{
	"auto": true, "claude": true, "codex": true, "gemini": true, "opencode": true, "droid": true,
}

var validTargetProviders = map[string]bool{
	"codex": true, "gemini": true, "opencode": true, "droid": true,
}
...
func saveTransfer(workDir, content, format, targetProvider string) string {
	...
	name := fmt.Sprintf("ctx-transfer-%d%s", os.Getpid(), ext)
	if targetProvider != "" {
		name = fmt.Sprintf("ctx-transfer-%s-%d%s", targetProvider, os.Getpid(), ext)
	}
...
func sendToProvider(content, provider string) (bool, string) {
	...
	var askCmd string
	if selfPath != "" {
		askCmd = filepath.Join(filepath.Dir(selfPath), "ask")
