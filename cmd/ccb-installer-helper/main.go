// ccb-installer-helper replaces the inline Python calls in install.sh
// with a single, dependency-free Go binary.
//
// Usage:
//
//	ccb-installer-helper <subcommand> [args...]
//
// Subcommands:
//
...
func usage() {
	fmt.Fprintln(os.Stderr, `Usage: ccb-installer-helper <subcommand> [args...]
...
func cmdRemoveTmuxBlock(args []string) error {
	...
	//   # ====...
	reNew := regexp.MustCompile(`(?s)\n*# =+\n# CCB \(Claude Code Bridge\) tmux configuration.*?# End of CCB tmux configuration\n# =+`)
	content = reNew.ReplaceAllString(content, "")

	// Legacy format: everything from "# CCB tmux configuration" to EOF.
