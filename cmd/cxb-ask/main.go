...
func cleanupTaskLogs(logDir string) {
	...
	pattern := filepath.Join(logDir, "cxb-ask-*.log")
...
func maybeStartUnifiedDaemon() bool {
	...
	if selfPath != "" {
		...
		local := filepath.Join(selfDir, "cxb-askd")
		if _, err := os.Stat(local); err == nil {
			candidates = append(candidates, local)
		}
	}
	if found, err := exec.LookPath("cxb-askd"); err == nil {
...
func sendViaUnifiedDaemon(provider, message string, timeout float64, noWrap bool, caller string) int {
	...
	if state == nil {
		...
		if state == nil {
			fmt.Fprintln(os.Stderr, "[ERROR] Unified cxb-askd daemon not running")
	...
	if port == 0 {
		fmt.Fprintln(os.Stderr, "[ERROR] Invalid cxb-askd state")
		return cliutil.ExitError
	}
...
func usage() {
	fmt.Fprintln(os.Stderr, "Usage: cxb-ask <provider> [options] <message>")
...
func run(argv []string) int {
	...
	if useUnifiedDaemon() {
		stateFile := runtime.StateFilePath("cxb-askd.json")
		if !rpc.PingDaemon("cxb-ask", 0.5, stateFile) && !maybeStartUnifiedDaemon() {
			fmt.Fprintln(os.Stderr, "[ERROR] Unified cxb-askd daemon not running")
			return cliutil.ExitError
		}
	}
	...
	logFile := filepath.Join(logDir, fmt.Sprintf("cxb-ask-%s-%s.log", provider, taskID))
	statusFile := filepath.Join(logDir, fmt.Sprintf("cxb-ask-%s-%s.status", provider, taskID))
