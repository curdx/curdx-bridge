...
func buildToolDefs() []map[string]any {
	...
	for _, provider := range providerOrder {
		defs = append(defs, map[string]any{
			"name":        fmt.Sprintf("ccb_ask_%s", provider),
			"description": fmt.Sprintf("Submit a background request to %s (CCB).", provider),
			"inputSchema": askSchema(),
		})
		defs = append(defs, map[string]any{
			"name":        fmt.Sprintf("ccb_pend_%s", provider),
			"description": fmt.Sprintf("Fetch the result of a background %s request.", provider),
			"inputSchema": pendSchema(),
		})
		defs = append(defs, map[string]any{
			"name":        fmt.Sprintf("ccb_ping_%s", provider),
			"description": fmt.Sprintf("Check availability for %s in CCB.", provider),
	...
	for _, a := range aliasTools {
		...
		defs = append(defs, map[string]any{
			"name":        a.name,
			"description": fmt.Sprintf("Alias for ccb_%s_%s.", a.kind, a.provider),
			"inputSchema": schema,
		})
	}
	return defs
...
func spawnBackground(cmd []string, message string, metaFilePath string) *int {
	...
	env := os.Environ()
	// Set CCB_CALLER in env
	found := false
	for i, e := range env {
		if strings.HasPrefix(e, "CCB_CALLER=") {
			env[i] = "CCB_CALLER=" + ccbCaller
			found = true
			break
		}
	}
	if !found {
		env = append(env, "CCB_CALLER="+ccbCaller)
	}
...
func handleToolCall(name string, args map[string]any) map[string]any {
	...
	if strings.HasPrefix(name, "ccb_ask_") {
		return submitTask(provider, args)
	}
	if strings.HasPrefix(name, "ccb_pend_") {
		return pendTask(provider, args)
	}
	if strings.HasPrefix(name, "ccb_ping_") {
		return pingProvider(provider, args)
	}
	return toolError(fmt.Sprintf("unknown tool: %s", name))
}
...
func main() {
	...
	if v := os.Getenv("CCB_DELEGATION_CACHE_DIR"); v != "" {
		cacheDir = v
	} else {
		cacheDir = filepath.Join(homeDir, ".cache", "ccb", "delegation")
	}
	logPath = filepath.Join(cacheDir, "mcp-server.log")

	if v := os.Getenv("CCB_DELEGATION_TTL_S"); v != "" {
