...
func usage() {
	fmt.Fprintln(os.Stderr, "Usage: cxb-claude-askd [OPTIONS]")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "cxb-claude-ask daemon (Claude)")
	fmt.Fprintln(os.Stderr, "")
...
func main() {
	...
	if shutdown {
		sf := stateFile
		if sf == "" {
			sf = runtime.StateFilePath("cxb-claude-askd.json")
		}
		ok := rpc.ShutdownDaemon("cxb-claude-ask", 5.0, sf)
		if ok {
			os.Exit(0)
		}
		os.Exit(1)
	...
	fmt.Fprintf(os.Stderr, "cxb-claude-askd: starting on %s:%d\n", host, port)
	if stateFile != "" {
		fmt.Fprintf(os.Stderr, "cxb-claude-askd: state file: %s\n", stateFile)
	}
	...
	fmt.Fprintln(os.Stderr, "cxb-claude-askd: daemon implementation pending (use Python laskd for now)")
	os.Exit(1)
}
